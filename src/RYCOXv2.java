    ...
    static void initialise() {
        ...
        if (exist_cl_data == false) {    //if client's file do not exist
            ...
            try {
                ...
                prgList.add(new TVProgramme("F4", "Pokemon", "Ash Ketchum and his friends (human and Pok��mon) that he makes on the way.", "Japan", "1/4/1997", "N/A", "ACTIVE", "U", "Comedy", "2stars"));
            ...
            if (usernameVal) {
                for (int i = 0; i < userList.size(); i++) {
                    if ((password.equals(userList.get(u).getPassword())) == false) {
                        ...
                        logList.addLast(log);
                        userList.get(u).setLastLoggedIn();
                        mainFrame.removeAll();
                        mainUI();
                        mainFrame.repaint();
                        mainFrame.validate();
